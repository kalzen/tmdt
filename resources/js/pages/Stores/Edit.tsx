...
interface Props {
    ...
    storeCatalogueIds: number[];
}

export default function EditStore({ store, users, catalogues, storeCatalogueIds }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        name: store.name || '',
        slug: store.slug || '',
        description: store.description || '',
        user_id: store.user_id ? store.user_id.toString() : '',
    ...
    const [selectedCatalogues, setSelectedCatalogues] = useState<number[]>(storeCatalogueIds || []);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${__('admin.edit_store', 'Edit Store')}: ${store.name}`} />
            
            <div className="space-y-6 p-6">
                <div className="flex items-center justify-between">
                    <HeadingSmall 
                        title={`${__('admin.edit_store', 'Edit Store')}: ${store.name}`} 
                        description={__('admin.edit_store_description', 'Update store information')} 
                    />
                </div>
